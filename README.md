...
- **价目表可视化编辑**：设置页新增独立「喵缓存账单」标签页（与「通用」「模型」平级），添加 / 修改 / 恢复预填，改完即时生效，无需重启。**新增条目时不用手打名字**：供应商直接从 DSH 已配置的列表里挑（● 已启用排前面，○ 可配置在后，另留「手填」与「留空=通配」），模型点「获取模型」当场向供应商问清单（自定义中转会带上它在 DSH 里配置的 baseURL；适配器不支持自动获取时，下拉里仍列出价目表里记录过的模型）。包根 `rates.yml` 是插件自带的预填层（provider/model 按 API 官方写法，峰谷（days × ranges 叉乘）或一口价（const）三选一，时区按计费方账单时区（IANA 名）），改它才需要重启 `dsh web`；条目写坏自动跳过并警告，绝不弄崩 DSH。
