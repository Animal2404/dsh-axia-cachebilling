...
function PriceSourcesPanel(props) {
  ...
  const meta = summary.fromCatalog ? `${summary.rows.length} 个来源 · ${summary.total ?? 0} 条价目${updatedAt ? ` · 内容版本 ${updatedAt}` : ""} · 最近拉取 ${fmtClock(props.lastFetched ?? null)}${props.justRefreshed ? " · 已刷新 ✓" : ""}` : "兜底清单 · 未能读取价目表";
  return el(
    "div",
...
function BillingCard(props) {
  ...
  const refreshFlashTimer = React.useRef(null);
  ...
  const refreshPrices = async (force, thenApply) => {
    ...
    if (catalog2 !== null) {
      ...
      if (force) {
        ...
        refreshFlashTimer.current = window.setTimeout(() => setJustRefreshed(false), 2e3);
      }
    }
    if (catalog2 === null) {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      if (catalog2 !== null) setLastFetched(Date.now());
      setPrices(
  ...
  const editor = draft === null ? null : el(
    ...
    el(
      ...
      prices.catalog !== null ? el("span", { className: "axia_set_hint" }, `内容版本 ${prices.catalog.updatedAt ?? "—"} · 最近拉取 ${fmtClock(lastFetched)}${justRefreshed ? " · 已刷新 ✓" : ""}`) : null
    ),
