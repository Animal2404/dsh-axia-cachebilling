...
function collectPriceSources(catalog) {
...
function PriceSourcesPanel(props) {
  const summary = collectPriceSources(props.catalog);
  const updatedAt = props.catalog?.updatedAt;
  const meta = summary.fromCatalog ? `${summary.rows.length} 个来源 · ${summary.total ?? 0} 条价目${updatedAt ? ` · 内容版本 ${updatedAt}${lastFetchedAt !== null ? ` · 最近拉取 ${fmtClock(lastFetchedAt)}` : ""}` : ""}` : "兜底清单 · 未能读取价目表";
  return el(
    "div",
    ...
    summary.fromCatalog ? null : el("p", { className: "axia_set_src_warn" }, "未能读取价目表：下面列出的是内置的 6 个官方来源，模型数未知。"),
    el("div", { className: "axia_set_src_list" }, summary.rows.map(
      (row) => el(
        "div",
...
function BillingCard(props) {
  ...
  const fmtClock2 = (t) => t === null ? "—" : new Date(t).toLocaleTimeString("zh-CN", { hour12: false });
  let lastFetchedAt2 = null;
  ...
  const refreshPrices = async (force, thenApply) => {
    ...
    if (catalog2) {
      lastFetchedAt2 = Date.now();
      setLastFetched(lastFetchedAt2);
      if (force) {
        setJustRefreshed(true);
        window.setTimeout(() => setJustRefreshed(false), 2e3);
      }
    }
    if (catalog2 === null) {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      setPrices(
        (prev) => catalog2 === null ? { ...prev, status: "error", note: prev.note ?? "价格目录拉取失败：稍后再点一次「刷新价格」，或直接手填" } : { ...prev, status: "ready", catalog: catalog2 }
      );
    })();
    return () => {
      cancelled = true;
  ...
  const editor = draft === null ? null : el(
    ...
    el(
      ...
      prices.catalog !== null ? el("span", { className: "axia_set_hint" }, `目录内容版本 ${prices.catalog.updatedAt ?? "—"} · 最近拉取 ${fmtClock2(lastFetched)}${justRefreshed ? " · 已刷新 ✓" : ""}`) : null
    ),
  ...
  return el(
    ...
    showSources ? el(PriceSourcesPanel, {
...
.axia_tileval { color: var(--dsw-alias-label-primary); flex: none; font-size: calc(10px * var(--axia-fs,1)); font-variant-numeric: tabular-nums; font-weight: 600; line-height: calc(14px * var(--axia-fs,1)); text-align: right; }
...
.axia_legendlab { color: var(--dsw-alias-label-secondary); flex: none; font-size: calc(9.5px * var(--axia-fs,1)); line-height: calc(12px * var(--axia-fs,1)); min-width: calc(68px * var(--axia-fs,1)); order: 1; white-space: nowrap; }
