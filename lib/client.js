...
.meowcb_set_btn_primary{background:var(--dsw-alias-button-primary-fill,#4D6BFE);border-color:transparent;color:#fff;font-weight:600}
.meowcb_set_btn_primary:hover{background:var(--dsw-alias-button-primary-fill,#4D6BFE);color:#fff;filter:brightness(1.08)}
  ...
  const editor = draft === null ? null : el(
    ...
    draft.isPeak ? el(
      ...
      field(
        "峰时段（[天][时段]，多组用 + 连接）",
        el("input", {
          className: "meowcb_set_input meowcb_set_input_mono" + (draft.whenText.trim() ? "" : " meowcb_set_input_err"),
      ...
      tzFor(draft.provider).auto ? el(
        ...
        "时区（IANA）",
        el("input", {
          className: "meowcb_set_input meowcb_set_input_mono" + (draft.timezone.trim() ? "" : " meowcb_set_input_err"),
