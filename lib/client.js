...
function renderDetails(doc, put, view) {
  ...
  put(modelLine);
  const money = (value) => `${view.currency === "USD" ? "$" : "¥"}${formatAmount(Number.isFinite(value) ? value : 0)}`;
  const section = (title) => put(secHead(doc, "#60a5fa", title));
